...
std::vector<torch::Tensor> attention_forward(
    ...
    return flash_attention(q, k.transpose(-2, -1), v, batch_size, head_num, token_num, features);
}
...
std::vector<torch::Tensor> attention_backward(
    ...
    auto esums_expand = esums.unsqueeze(-1).expand_as(qk);
    auto attention_weights = torch::exp(qk/std::sqrt(features))/esums_expand;
