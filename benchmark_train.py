...
class SentimentNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.attention_type in ["cpp", "fused"]:
            embeds = embeds.half()
        import math
        ...
        start.record()
        if self.attention_type in ["cpp", "cuda"]:
            att_out = self.att(embeds, embeds, embeds)
        elif self.attention_type == "fused":
            att_out = self.att(embeds, embeds, embeds, 1/(math.sqrt(hidden_size)))
        end.record()
